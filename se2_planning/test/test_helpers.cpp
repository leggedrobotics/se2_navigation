...
void addObstacles(std::function<bool(double, double)> isObstacle, const std::string& layer, grid_map::GridMap* gridMap) {
...
grid_map::GridMap createGridMap(double length, double width, double resolution, std::function<bool(double, double)> isAnObstacle) {
...
bool isPathCollisionFree(const se2_planning::ReedsSheppPath& path, const se2_planning::StateValidator& validator) {
...
bool isStartAndGoalStateOK(const se2_planning::ReedsSheppPath& path, const se2_planning::ReedsSheppState& start,
...
se2_planning::ReedsSheppState randomState(const se2_planning::OmplReedsSheppPlannerParameters& parameters) {
...
se2_planning::OmplReedsSheppPlannerParameters createRectangularStateSpaceWithDefaultParams(double stateBound) {
...
void setupPlanner(const se2_planning::OmplReedsSheppPlannerParameters& parameters, se2_planning::OmplReedsSheppPlanner* planner) {
  planner->setParameters(parameters);
  planner->initialize();
  setCostThreshold(planner);
}

bool simplePlanBetweenRandomStartAndGoalTest(se2_planning::OmplReedsSheppPlanner& planner,
