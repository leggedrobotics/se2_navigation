...
TEST(Planning, OmplReedsSheppPlanner) {
  ...
  se2_planning::OmplReedsSheppPlannerParameters parameters = createRectangularStateSpaceWithDefaultParams(10.0);
...
TEST(PlanningObstacle, PlanningWithStateValidator) {
  ...
  // create environment
  std::function<bool(double, double)> isAnObstacle = [](double x, double y) { return isInsideRectangle(x, y, 0.0, 0.0, 10.0, 10.0); };
  grid_map::GridMap gridMap = createGridMap(40.0, 40.0, 0.1, isAnObstacle);

  // setup planner
  se2_planning::OmplReedsSheppPlannerParameters parameters = createRectangularStateSpaceWithDefaultParams(20.0);
  se2_planning::OmplReedsSheppPlanner planner;
  setupPlanner(parameters, &planner);
  ...
  if (::testing::Test::HasFailure()) {
    std::cout << "\n Test PlanningObstacle, PlanningWithStateValidator failed with seed: " << seed << std::endl;
  }
}
...
TEST(PlanningObstacle, PlanningWithStateValidatorHarder) {
  ...
  if (::testing::Test::HasFailure()) {
    std::cout << "\n Test PlanningObstacle, PlanningWithStateValidatorHarder failed with seed: " << seed << std::endl;
  }
}
...
TEST(PlanningObstacle, PlanningWithLazyStateValidator) {
  ...
  if (::testing::Test::HasFailure()) {
    std::cout << "\n Test PlanningObstacle, PlanningWithLazyStateValidator failed with seed: " << seed << std::endl;
  }
}
...
TEST(PlanningObstacle, PlanningWithLazyStateValidatorHarder) {
  ...
  if (::testing::Test::HasFailure()) {
    std::cout << "\n Test PlanningObstacle, PlanningWithLazyStateValidatorHarder failed with seed: " << seed << std::endl;
  }
}
