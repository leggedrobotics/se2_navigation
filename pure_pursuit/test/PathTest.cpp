...
TEST(Path, ExtendingPathSegment)
{
  ...
  for (unsigned int i = 0; i < numCasesPerTest; ++i) {
    ...
    const unsigned int numPointsBefore = pathSegment.segment_.size();
    ...
    EXPECT_EQ(numPointsBefore + 1, pathSegment.segment_.size());
    int id = pathSegment.segment_.size() - 1;
    const auto lastPoint = pathSegment.segment_.at(id--).position_;
    const auto secondLastPoint = pathSegment.segment_.at(id--).position_;
    const auto thirdLastPoint = pathSegment.segment_.at(id).position_;
    EXPECT_TRUE(
