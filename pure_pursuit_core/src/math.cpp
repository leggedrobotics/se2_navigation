...
double AverageFilter::filterInputValue(double value)
{
  ...
  const double retVal = weightMostRecentMeasurement_ * value
      + (1.0 - weightMostRecentMeasurement_) * filteredValuePrev_;
...
Point chooseCorrectLookaheadPoint(const Intersection& intersection, const Vector& desiredHeading,
                                  const Point& origin)
{
...
void findIdOfFirstPointsCloserThanLookaheadAndFirstPointsFartherThanLookahead(
    const PathSegment& pathSegment, const Point& anchorPoint, unsigned int startingPoint,
    double lookaheadDistance, unsigned int* closerPointId, unsigned int* fartherPointId)
{
  ...
  int pointCloserThanLookaheadId = bindIndexToRange(pointFartherThanLookaheadId - 1, 0,
                                                    nPoints - 1);
...
bool computeLookaheadAngle(const Point& lookaheadPoint, const Point& anchorPoint,
                           const Vector& heading, DrivingDirection drivingDirection,
...
double computeSteeringAngleCmd(double lookaheadAngle, double lookaheadDistance,
                               double anchorDistancte, double wheelBase)
{
  double steeringAngle;

  steeringAngle = -std::atan(
      wheelBase * std::sin(lookaheadAngle)
          / (lookaheadDistance * 0.5 + anchorDistancte * std::cos(lookaheadAngle)));

  return steeringAngle;
}

Point computeAnchorPoint(const RobotState& robotState, double anchorDistance,
...
bool computeLookaheadPoint(unsigned int closestPointOnPathSegmentId, double lookaheadDistance,
                           const RobotState& robotState, DrivingDirection drivingDirection,
                           const PathSegment& pathSegment, Point* lookaheadPoint)
{
  const Point anchorPoint = computeAnchorPoint(robotState, lookaheadDistance,
                                               robotState.desiredDirection_);
  unsigned int fartherPointId, closerPointId;
  findIdOfFirstPointsCloserThanLookaheadAndFirstPointsFartherThanLookahead(
      pathSegment, anchorPoint, closestPointOnPathSegmentId, lookaheadDistance, &closerPointId,
      &fartherPointId);

  const Line line(pathSegment.point_.at(closerPointId).position_,
                  pathSegment.point_.at(fartherPointId).position_);
  ...
  switch (intersection.solutionCase_) {
    ...
    case Intersection::SolutionCase::TWO_SOLUTIONS: {
      const Vector heading = computeDesiredHeadingVector(robotState.pose_.yaw_,
                                                         robotState.desiredDirection_);
