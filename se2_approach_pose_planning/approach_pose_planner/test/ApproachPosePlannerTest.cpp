...
bool isStartAndGoalStateOK(const se2_planning::ReedsSheppPath& path, const se2_planning::ReedsSheppState& start,
                           const se2_planning::ReedsSheppState& goal, const se2_planning::ApproachPosePlannerParameters& params) {
  if (path.numPoints() < 2) {
    return false;
  }
  const auto firstState = path.segment_.front().point_.front();
...
TEST(ApproachPosePlanner, PlanningGoal) {
  ...
  parameters.plannerImplParams_ = se2_planning_test::createRectangularStateSpaceWithDefaultParams(15.0);
